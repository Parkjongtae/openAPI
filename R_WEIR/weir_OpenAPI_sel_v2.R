...
stday=read.table("DATE.txt",header=F,stringsAsFactors = F)
...
url<-paste("http://opendata.kwater.or.kr/openapi-data/service/pubd/dam/sluicePresentCondition/hour/list?damcode=",stionnum[j,1],"&stdt=",stday[i,1],"&eddt=",stday[i,1],"&numOfRows=99&pageNo=1&ServiceKey=xcipP6iDVOmrjwn0NzrEoOr5Z%2BPYZXWIrdbsTJ3fDwkzUggZ24yV7P2Tu2enu%2BPz8Om7YAyk2S%2FUOR5KRo4uiw%3D%3D")
url<-gsub(" ","",url)
print(url)
Sys.sleep(1)
...
rm(df)
  ...
  jdaybaseyr<-substr(stday[1,1],1,4)
  ...
  savefile<-paste(stionnum[j,1],"_",stionnum[j,2],".csv")
